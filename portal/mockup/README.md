...
The top nav is wired between Home ↔ Palette ↔ Vendors ↔ Budget. The remaining four links (Checklist, Timeline, Floor Plan, "Guest List, RSVP, and Seating") still point to `#` until those pages exist.
...
| Top nav: 4 of 8 still link to `#` | Wire to real page paths as each sub-page is built (Home + Palette + Vendors + Budget done) |
...
| Budget + vendors are read-only HTML | Make editable in the real product — Zoe should be able to add/edit/delete categories, line items, vendors, and amounts per couple |
