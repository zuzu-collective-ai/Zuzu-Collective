...
The top nav is wired between Home ↔ Palette ↔ Vendors ↔ Checklist ↔ Budget ↔ Timeline. The remaining two links (Floor Plan, "Guest List, RSVP, and Seating") still point to `#` until those pages exist.
...
| Top nav: 2 of 8 still link to `#` | Wire to real page paths as each sub-page is built (Home + Palette + Vendors + Checklist + Budget + Timeline done) |
