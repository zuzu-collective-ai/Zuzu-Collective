...
The top nav is wired between Home ↔ Palette ↔ Vendors ↔ Checklist ↔ Budget. The remaining three links (Timeline, Floor Plan, "Guest List, RSVP, and Seating") still point to `#` until those pages exist.
...
| Top nav: 3 of 8 still link to `#` | Wire to real page paths as each sub-page is built (Home + Palette + Vendors + Checklist + Budget done) |
