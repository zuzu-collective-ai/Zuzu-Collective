...
- `floor-plan.html` — three top-down venue schematics drawn in pure CSS (no SVG, no canvas, no images). Each schematic is a 16:10 grid-overlay container with absolutely-positioned zones: round tables, the Finn-filled head table, hatched dance floor, Tobacco-filled bar, dashed doorways, dotted chair blocks, hollow cocktail high-tops. Each schematic gets a compass marker (N&nbsp;↑), a scale bar, edge labels (cardinal directions / adjacency), and a numbered key alongside that names every zone in italic display + a one-line detail. The three spaces are **The Ocean Lawn** (ceremony, 60 seats, 2,400 sq ft, outdoor — arch + aisle + 48 chairs in 6 rows), **The Upper Lawn &amp; Loggia** (cocktail hour, 120 capacity, 2,200 sq ft — main bar + Carmel Strings trio + 6 high-tops + signature pour station), and **The Garden Ballroom** (reception, 124 capacity, 3,800 sq ft — DJ booth + dance floor + bar + 9 round tables + the Cypress head table + cake corner + back-of-house catering ingress). Print-friendly so the day-of plan prints cleanly. Section 08.

The top nav is wired across all eight sections — Home ↔ Palette ↔ Vendors ↔ Checklist ↔ Budget ↔ Timeline ↔ Floor Plan ↔ Guest List. No stub links remain.
